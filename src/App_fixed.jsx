...
function OnlineBoard({roomId,myColor,myName,onLeave}){
  ...
  const isSikaMe=gs?.sika===myColor; // أنا المحصور
      ...
      if(sel){
        ...
        if(ok){
          ...
          await dbPatch(`rooms/${roomId}`,{
            board:b2s(nb),turn:nt,winner:win||null,
  ...
  if(oppGone)msg="⚠️ الخصم خرج!";
    ...
    else if(isSikaMe)msg="أنت محصور! اضغط 🚪 طلب سكة";
        ...
        {board.map((row,r)=>(
          <div key={r} style={{display:"flex"}}>
            {row.map((piece,c)=>{
              ...
              return(
                <div key={c} onClick={()=>click(r,c)} style={{width:C,height:C,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",cursor:(isMyTurn||isSikaOpp)?"pointer":"default",background:bg,border:`1px solid ${border}`,borderRadius:3,boxSizing:"border-box",opacity:dim?0.3:1,transition:"opacity 0.2s"}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"center"}}>
        {isSikaMe&&!gs?.winner&&(
          <button onClick={async()=>{
            // المحصور يطلب السكة - الدور ينتقل للخصم عشان يفتح
            await dbPatch(`rooms/${roomId}`,{
              turn:opp,
