...
function OnlineBoard({roomId,myColor,playerName,onLeave}){
  ...
  const handleCell=async(r,c)=>{
    ...
    if(gs.phase==="placement"){
      ...
      await dbUpdate(`rooms/${roomId}`,{board:boardToStr(nb),placed:newPlaced,turnPlaced:nextTurn,currentPlayer:nextPlayer,phase:newPhase,firstPlacer:gs.firstPlacer||myColor,version:(gs.version||0)+1});
      ...
      if(sel){
        ...
        if(valid){
          ...
          await dbUpdate(`rooms/${roomId}`,{board:boardToStr(nb),currentPlayer:winner?null:hasMore?myColor:other(myColor),winner:winner||null,locked:hasMore?`${r},${c}`:null,version:(gs.version||0)+1});
        ...
        const locked=gs.locked?gs.locked.split(",").map(Number):null;if(locked&&!(locked[0]===r&&locked[1]===c))return;
