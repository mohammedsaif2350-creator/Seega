...
async function dbUpdate(path,data){
  try{
    ...
    const timeout=setTimeout(()=>ctrl.abort(),5000); // timeout 5 ثواني
    await fetch(`${DB_URL}/${path}.json`,{method:"PATCH",body:JSON.stringify(data),headers:{"Content-Type":"application/json"},signal:ctrl.signal});
    clearTimeout(timeout);
  }catch(e){}
}
function genCode(){return Math.random().toString(36).substring(2,8).toUpperCase();}
// اللوح كـ string عشان Firebase ما يحوله لـ object
function boardToStr(b){return b.map(r=>r.map(c=>c===null?".":c===WHITE?"W":"B").join("")).join("|");}
function strToBoard(s){if(!s)return Array(5).fill(null).map(()=>Array(5).fill(null));return s.split("|").map(r=>r.split("").map(c=>c==="."?null:c==="W"?WHITE:BLACK));}
function emptyBoardStr(){return ".....|.....|.....|.....|.....";}
...
function OnlineBoard({roomId,myColor,playerName,onLeave}){
  ...
  const [sikaWaiting,setSikaWaiting]=useState(false);
  const vRef=useRef(null);
  const hbRef=useRef(null);
  const CELL=58,PS=42;
  const oppColor=other(myColor);

  // Firebase SSE - أسرع من polling وأكثر استقراراً
  useEffect(()=>{
    let es = null;
    let hbInterval = null;
    ...
    const connect = () => {
      if (!active) return;
      try {
        es = new EventSource(`${DB_URL}/rooms/${roomId}.json`);
        ...
        es.addEventListener("put", (e) => {
          ...
          try {
            ...
            if (data.version !== vRef.current) {
              ...
              setGs(data);
              // فحص heartbeat الخصم
              const oppHb = data.hb?.[oppColor] || 0;
              if (Date.now() - oppHb > 10000 && data.players?.[oppColor]) setOppLeft(true);
        ...
        es.addEventListener("patch", (e) => {
          if (!active) return;
          try {
            // عند patch نعمل GET خفيف للبيانات الكاملة
            fetch(`${DB_URL}/rooms/${roomId}.json`)
              .then(r => r.json())
              .then(data => {
        ...
        // Fallback للـ polling لو SSE ما يشتغل
        const poll = async () => {
          if (!active) return;
          try {
        ...
        poll();
        hbInterval = setInterval(poll, 1500);
      }
    };

    connect();

    // Heartbeat كل 4 ثواني
    const sendHb = () => {
      try { dbUpdate(`rooms/${roomId}/hb`, {[myColor]: Date.now()}); } catch(e) {}
    };
    sendHb();
    hbInterval = setInterval(sendHb, 4000);
  ...
  }, [roomId, myColor, oppColor]);

  const isMyTurn=gs?.currentPlayer===myColor&&!gs?.winner;
  // اللوح مخزون كـ string - آمن من مشكلة Firebase array→object
  const board=gs?.board?strToBoard(gs.board):emptyBoard();
  const placedData=gs?.placed||{};
  const myInHand=12-(placedData[myColor]||0);
  const oppInHand=12-(placedData[oppColor]||0);
  const myOnBoard=countPieces(board,myColor);
  const oppOnBoard=countPieces(board,oppColor);
  const isBlocked_=()=>isBlocked(board,myColor);

  const handleCell=async(r,c)=>{
    if(!isMyTurn||!gs) return;
    if(sikaWaiting) return;

    if(gs.phase==="placement"){
      if(board[r][c]!==EMPTY||(r===2&&c===2)) return;
      const myPlaced=placedData[myColor]||0;
      const turnPlaced=gs.turnPlaced||0;
      if(myPlaced>=12||turnPlaced>=2) return;
      const nb=board.map(r=>[...r]);
      nb[r][c]=myColor;
      const newPlaced={...placedData,[myColor]:myPlaced+1};
      const newTurn=turnPlaced+1;
      let nextPlayer=myColor,nextTurn=newTurn,newPhase="placement";
      if(newTurn>=2){
        nextPlayer=other(myColor);nextTurn=0;
        const total=(newPlaced[WHITE]||0)+(newPlaced[BLACK]||0);
        if(total>=24){newPhase="movement";nextPlayer=other(gs.firstPlacer||myColor);}
      }
      await dbUpdate(`rooms/${roomId}`,{
        board:boardToStr(nb),placed:newPlaced,turnPlaced:nextTurn,
        currentPlayer:nextPlayer,phase:newPhase,
        firstPlacer:gs.firstPlacer||myColor,version:(gs.version||0)+1
      });
      ...
      if(sel){
        ...
        if(valid){
          const nb=board.map(r=>[...r]);
          const caps=getCaptures(board,sel[0],sel[1],r,c);
          nb[r][c]=myColor;nb[sel[0]][sel[1]]=EMPTY;
          for(const[cr,cc]of caps)nb[cr][cc]=EMPTY;
          setSel(null);setVmoves([]);setLastCaps(caps);
          setAnim([[r,c]]);setTimeout(()=>setAnim([]),400);
          const winner=checkWin(nb,myColor);
          const hasMore=caps.length>0&&getCaptureMoves(nb,r,c).length>0;
          // تحقق السكة للخصم
          const nextPlayer=winner?null:hasMore?myColor:other(myColor);
          // فحص الحصار
          const oppColor2=other(myColor);
          let oppBlocked=false;
          if(!winner&&!hasMore){
            oppBlocked=true;
            for(let br=0;br<5&&oppBlocked;br++)
              for(let bc=0;bc<5&&oppBlocked;bc++)
                if(nb[br][bc]===oppColor2)
                  for(const[dr,dc]of[[-1,0],[1,0],[0,-1],[0,1]]){
                    const nr2=br+dr,nc2=bc+dc;
                    if(nr2>=0&&nr2<5&&nc2>=0&&nc2<5&&nb[nr2][nc2]===null){oppBlocked=false;break;}
                  }
          }
          await dbUpdate(`rooms/${roomId}`,{
            board:boardToStr(nb),
            ...
            winner:winner||null,
            locked:hasMore?`${r},${c}`:null,
            sikaRequest:oppBlocked?other(myColor):null,
            version:(gs.version||0)+1
          });
        } else if(board[r][c]===myColor){
          const locked=gs.locked?gs.locked.split(",").map(Number):null;
  ...
  // السكة: أنا المحصور
  const requestSika=async()=>{
    await dbUpdate(`rooms/${roomId}`,{sikaRequest:myColor,currentPlayer:oppColor,version:(gs.version||0)+1});
    setSikaWaiting(false);
  };

  // السكة: أنا الخصم أختار حجر
  const [sikaStep,setSikaStep]=useState(null); // null | [r,c]
  const handleSikaCell=async(r,c)=>{
    const blocked=gs.sikaRequest;
    if(!blocked||blocked===myColor) return;
    if(!sikaStep){
      if(board[r][c]!==myColor){return;}
      if(getValidMoves(board,r,c).length===0){return;}
      setSikaStep([r,c]);
    } else {
      if(board[r][c]!==EMPTY) return;
      const dr=Math.abs(r-sikaStep[0]),dc=Math.abs(c-sikaStep[1]);
      if(!((dr===1&&dc===0)||(dr===0&&dc===1))) return;
      const nb=board.map(r=>[...r]);
      nb[r][c]=myColor;nb[sikaStep[0]][sikaStep[1]]=EMPTY;
      setSikaStep(null);
      await dbUpdate(`rooms/${roomId}`,{
        board:boardToStr(nb),currentPlayer:blocked,
        sikaRequest:null,version:(gs.version||0)+1
      });
    }
  };
  ...
  const isSikaGiver=gs?.sikaRequest&&gs.sikaRequest!==myColor;
  const isSikaBlocked=gs?.sikaRequest===myColor;

  // ── مؤقت 30 ثانية ──────────────────────────────────────
  const [onlineTimer,setOnlineTimer]=useState(30);
  const [timerOn,setTimerOn]=useState(false);
  const timerRef=useRef(null);
  const curRef2=useRef(gs?.currentPlayer);
  curRef2.current=gs?.currentPlayer;

  useEffect(()=>{
    if(!gs||gs.phase!=="movement"||gs.winner) return;
    if(gs.currentPlayer!==myColor) return; // المؤقت بس لدوري
    let count=30;
    setOnlineTimer(30);setTimerOn(true);
    timerRef.current=setInterval(()=>{
      count--;setOnlineTimer(count);
      if(count<=0){
        clearInterval(timerRef.current);setTimerOn(false);
        // انتهى الوقت - انتقال الدور
        dbUpdate(`rooms/${roomId}`,{
          currentPlayer:other(myColor),
          version:(gs.version||0)+1
        });
      }
    },1000);
    return()=>{clearInterval(timerRef.current);setTimerOn(false);};
  },[gs?.currentPlayer,gs?.phase,gs?.winner]);

  let msg="يتم التحميل...";
  if(oppLeft) msg="⚠️ الخصم خرج من اللعبة!";
  else if(gs){
    if(gs.winner) msg=gs.winner===myColor?"🏆 فزت! 🎉":"💔 خسرت!";
    else if(isSikaBlocked) msg="أنت محصور! اضغط 🚪 طلب سكة";
    else if(isSikaGiver) msg=`${sikaStep?"اختر مربعاً فاضياً":"اختر حجراً من حجارك"} لتفتح السكة`;
    else if(isMyTurn) msg=gs.phase==="placement"?`دورك ← ضع حجرين (${myInHand} في اليد)`:"دورك ← اختر حجراً";
    else msg="⏳ دور خصمك...";
  }
  const handleCellFull=(r,c)=>{
    if(isSikaGiver){handleSikaCell(r,c);return;}
    handleCell(r,c);
  };
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:9,width:"100%",maxWidth:420}}>
      {oppLeft&&(
        <div style={{background:"rgba(200,80,20,0.2)",border:"1px solid rgba(220,100,40,0.5)",borderRadius:10,padding:"8px 16px",color:"#ffb080",fontFamily:"'Cairo',sans-serif",fontSize:"0.78rem",textAlign:"center",width:"100%"}}>
          ⚠️ الخصم خرج — يمكنك مغادرة اللعبة
        </div>
      )}
      <div style={{background:"rgba(0,0,0,0.3)",border:"1px solid rgba(212,168,67,0.2)",borderRadius:10,padding:"5px 16px",display:"flex",alignItems:"center",gap:8,justifyContent:"center"}}>
        ...
        {!oppLeft&&gs&&!gs.winner&&<div style={{width:6,height:6,borderRadius:"50%",background:"#44cc44",boxShadow:"0 0 6px #44cc44"}}/>}
      </div>
      ...
      <div style={{background:"linear-gradient(145deg,#2C1F0E,#1E1509)",border:"3px solid rgba(212,168,67,0.2)",borderRadius:12,padding:8,boxShadow:"0 20px 60px rgba(0,0,0,0.8)",opacity:!gs?0.5:1,position:"relative"}}>
        ...
        {Array(BOARD_SIZE).fill(null).map((_,r)=>(<div key={r} style={{display:"flex"}}>{Array(BOARD_SIZE).fill(null).map((_,c)=>{
          ...
          if(isSikaGiver&&!sikaStep&&board[r][c]===myColor&&getValidMoves(board,r,c).length>0){border="rgba(255,150,50,0.7)";bg="rgba(200,80,20,0.1)";}
          if(isSikaGiver&&sikaStep&&board[r][c]===EMPTY){border="rgba(255,150,50,0.5)";bg="rgba(200,80,20,0.08)";}
          if(sikaStep&&sikaStep[0]===r&&sikaStep[1]===c){border="rgba(255,200,50,0.8)";bg="rgba(200,160,20,0.15)";}
          return(<div key={c} onClick={()=>handleCellFull(r,c)} style={{width:CELL,height:CELL,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",cursor:isMyTurn?"pointer":"default",background:bg,border:`1px solid ${border}`,borderRadius:3,boxSizing:"border-box"}}>
      ...
      <div style={{display:"flex",gap:7,flexWrap:"wrap",justifyContent:"center"}}>
        {isSikaBlocked&&!gs?.winner&&(
          <button onClick={requestSika} style={{padding:"4px 12px",borderRadius:20,background:"linear-gradient(135deg,rgba(200,60,20,0.5),rgba(150,40,10,0.3))",border:"1px solid rgba(220,100,40,0.5)",color:"#ffb080",fontFamily:"'Cairo',sans-serif",fontSize:"0.7rem",cursor:"pointer",fontWeight:700,animation:"sikaBeat 1s infinite"}}>🚪 طلب سكة</button>
        )}
        {gs?.winner&&(
          <button onClick={async()=>{
            await dbUpdate(`rooms/${roomId}`,{board:boardToStr(emptyBoard()),phase:"placement",currentPlayer:WHITE,placed:{white:0,black:0},turnPlaced:0,firstPlacer:null,winner:null,locked:null,sikaRequest:null,version:(gs.version||0)+1});
          }} style={{padding:"4px 12px",borderRadius:20,background:"rgba(212,168,67,0.1)",border:"1px solid rgba(212,168,67,0.25)",color:"#C8A96E",fontFamily:"'Cairo',sans-serif",fontSize:"0.7rem",cursor:"pointer"}}>🔄 جديد</button>
        )}
        <button onClick={onLeave} style={{padding:"4px 12px",borderRadius:20,background:"rgba(200,50,50,0.12)",border:"1px solid rgba(200,50,50,0.3)",color:"#e08080",fontFamily:"'Cairo',sans-serif",fontSize:"0.7rem",cursor:"pointer"}}>← رجوع</button>
      </div>
...
function OnlineLobby({playerName,onBack}){
  ...
  const createRoom=async()=>{
    ...
    try{
      ...
      const initData={
        ...
        method:"PUT",
        body:JSON.stringify({...initData, board:boardToStr(emptyBoard())}),
  ...
  if(screen==="game")return <ErrorBoundary><OnlineBoard roomId={roomId} myColor={myColor} playerName={playerName} onLeave={()=>{setScreen("lobby");setRoomId("");}}/></ErrorBoundary>;
