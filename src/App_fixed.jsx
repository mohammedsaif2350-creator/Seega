  ...
  useEffect(()=>{
    const poll=async()=>{
      try{
        ...
        if(!data){setOppLeft(true);return;}
        if(data.version!==vRef.current){vRef.current=data.version;setGs(data);}
        // تحقق من heartbeat الخصم
        const oppHb=data.hb?.[oppColor]||0;
        if(Date.now()-oppHb>8000&&data.players?.[oppColor]) setOppLeft(true);
        else setOppLeft(false);
      }catch(e){}
    };
    poll();
    const t=setInterval(poll,800);
    // Heartbeat كل 3 ثواني
    hbRef.current=setInterval(async()=>{
      try{await dbUpdate(`rooms/${roomId}/hb`,{[myColor]:Date.now()});}catch(e){}
    },3000);
    // Heartbeat فوري
    try{dbUpdate(`rooms/${roomId}/hb`,{[myColor]:Date.now()});}catch(e){}
    return()=>{clearInterval(t);clearInterval(hbRef.current);}
