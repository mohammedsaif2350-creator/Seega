...
function OnlineBoard({roomId,myColor,myName,onLeave}){
  ...
  const isMyTurn=gs?.turn===myColor&&!gs?.winner&&!gs?.sika;
  ...
  const isSikaOpp=gs?.sika&&gs.sika!==myColor; // أنا الخصم أفتح
  ...
  const doSikaOpp=async(r,c)=>{
    ...
    if(!sikaFrom){
      ...
      setSikaFrom(null);
      await dbPatch(`rooms/${roomId}`,{board:b2s(nb),turn:gs.sika,sika:null,v:(gs.v||0)+1});
    }
  };
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10,width:"100%",maxWidth:420}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"center"}}>
        {isSikaMe&&!gs?.winner&&(
          <button onClick={()=>dbPatch(`rooms/${roomId}`,{turn:opp,sika:myColor,v:(gs.v||0)+1})} style={{padding:"5px 14px",borderRadius:20,background:"linear-gradient(135deg,rgba(200,60,20,0.5),rgba(150,40,10,0.3))",border:"1px solid rgba(220,100,40,0.5)",color:"#ffb080",fontFamily:"'Cairo',sans-serif",fontSize:"0.72rem",cursor:"pointer",fontWeight:700}}>🚪 طلب سكة</button>
        )}
        {gs?.winner&&(
          <button onClick={()=>{
