    ...
    if(gs.phase==="placement"){
      ...
      if(sel){
        ...
        if(valid){
          ...
          const oppBlocked=!winner&&!hasMore&&isBlocked(nb,other(myColor));
          await dbUpdate(`rooms/${roomId}`,{
            board:boardToStr(nb),
    ...
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:9,width:"100%",maxWidth:420}}>
      ...
      <div style={{display:"flex",gap:8}}>
        {isSikaBlocked&&!gs?.winner&&(
          <button onClick={requestSika} style={{padding:"5px 14px",borderRadius:20,background:"linear-gradient(135deg,rgba(200,60,20,0.5),rgba(150,40,10,0.3))",border:"1px solid rgba(220,100,40,0.5)",color:"#ffb080",fontFamily:"'Cairo',sans-serif",fontSize:"0.72rem",cursor:"pointer",fontWeight:700}}>🚪 طلب سكة</button>
        )}
