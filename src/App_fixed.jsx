  ...
  const HandPanel=({player})=>{
    ...
    return(
      <div style={{
        ...
        transition:"all .35s",backdropFilter:"blur(6px)",WebkitBackdropFilter:"blur(6px)",
      }}>
        {/* اسم اللاعب + الحجر */}
    ...
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:9,width:"100%"}}>
      ...
      <div style={{width:"100%",maxWidth:420}}>
        <div style={{
          ...
          padding:"9px 16px",backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",
...
// ── Firebase DB helpers ────────────────────────────────────
const DB="https://seja-game-498119-da6a3-default-rtdb.firebaseio.com";

// ── Firebase helpers - بسيطة وموثوقة ─────────────────────
...
// اللوح كـ string لتجنب مشكلة Firebase array→object
function b2s(b){return b.map(r=>r.map(c=>c===null?".":c==="white"?"W":"B").join("")).join("|");}
function s2b(s){
  if(!s||typeof s!=="string")return Array(5).fill(null).map(()=>Array(5).fill(null));
  return s.split("|").map(r=>r.split("").map(c=>c==="."?null:c==="W"?"white":"black"));
}
const emptyStr=()=>".....|.....|.....|.....|.....";

// قوانين اللعبة
function onlineValidMoves(board,r,c){
  const m=[];
  for(const[dr,dc]of[[-1,0],[1,0],[0,-1],[0,1]]){
    const nr=r+dr,nc=c+dc;
    if(nr>=0&&nr<5&&nc>=0&&nc<5&&board[nr][nc]===null)m.push([nr,nc]);
  }
  return m;
}
function onlineCaptures(board,fr,fc,tr,tc){
  const piece=board[fr][fc];
  if(!piece)return[];
  const opp=piece==="white"?"black":"white";
  const sim=board.map(r=>[...r]);
  sim[tr][tc]=piece;sim[fr][fc]=null;
  const caps=[];
  const safeSet=new Set(["0,0","0,4","4,0","4,4","2,2"]);
  for(const[dr,dc]of[[-1,0],[1,0],[0,-1],[0,1]]){
    const ar=tr+dr,ac=tc+dc,br=tr+2*dr,bc=tc+2*dc;
    if(ar>=0&&ar<5&&ac>=0&&ac<5&&br>=0&&br<5&&bc>=0&&bc<5&&
       sim[ar][ac]===opp&&sim[br][bc]===piece&&!safeSet.has(`${ar},${ac}`))
      caps.push([ar,ac]);
  }
  return caps;
}
function onlineCapMoves(board,r,c){
  const m=[];
  for(const[dr,dc]of[[-1,0],[1,0],[0,-1],[0,1]]){
    const nr=r+dr,nc=c+dc;
    if(nr>=0&&nr<5&&nc>=0&&nc<5&&board[nr][nc]===null&&onlineCaptures(board,r,c,nr,nc).length>0)m.push([nr,nc]);
  }
  return m;
}
function onlineCount(board,p){return board.flat().filter(c=>c===p).length;}
function onlineWin(board,p){
  const oc=onlineCount(board,p==="white"?"black":"white");
  if(oc<=1)return p;
  if(oc<=2&&onlineCount(board,p)>=6)return p;
  return null;
}
function onlineBlocked(board,p){
  for(let r=0;r<5;r++)for(let c=0;c<5;c++)
    if(board[r][c]===p&&onlineValidMoves(board,r,c).length>0)return false;
  return true;
}

// ── Online Board ───────────────────────────────────────────
function OnlineBoard({roomId,myColor,myName,onLeave}){
  ...
  const safe=new Set(["0,0","0,4","4,0","4,4","2,2"]);

  // Simple reliable polling - كل 1.5 ثانية بدون SSE
  useEffect(()=>{
    let alive=true;
    const poll=async()=>{
      if(!alive)return;
      const data=await dbRead(`rooms/${roomId}`);
      if(!alive)return;
      if(!data){setOppGone(true);return;}
      if(data.v!==vRef.current){
        vRef.current=data.v;
        setGs(data);
        // فحص اتصال الخصم
        const oppHb=data.hb?.[opp]||0;
        setOppGone(Date.now()-oppHb>12000);
      }
    };
    poll();
    ...
    const hb=setInterval(()=>{
      dbPatch(`rooms/${roomId}/hb`,{[myColor]:Date.now()});
    },4000);
    dbPatch(`rooms/${roomId}/hb`,{[myColor]:Date.now()});
    return()=>{alive=false;clearInterval(t);clearInterval(hb);};
  },[roomId,myColor,opp]);

  // مؤقت 30 ثانية - بس لدوري
  useEffect(()=>{
    if(!gs||gs.phase!=="movement"||gs.winner)return;
    if(gs.turn!==myColor){setTimerOn(false);clearInterval(timerRef.current);return;}
    let count=30;
    setTimer(30);setTimerOn(true);
    timerRef.current=setInterval(()=>{
      count--;setTimer(count);
      if(count<=0){
        clearInterval(timerRef.current);setTimerOn(false);
        setSel(null);setVmoves([]);
        dbPatch(`rooms/${roomId}`,{turn:opp,v:(gs.v||0)+1});
      }
    },1000);
    return()=>{clearInterval(timerRef.current);setTimerOn(false);};
  },[gs?.turn,gs?.phase,gs?.winner,myColor]);

  const board=gs?s2b(gs.board):Array(5).fill(null).map(()=>Array(5).fill(null));
  const isMyTurn=gs?.turn===myColor&&!gs?.winner&&!(gs?.sika===myColor); // ليس دوري لو أنا المحصور
  const placed=gs?.placed||{white:0,black:0};
  const myHand=12-(placed[myColor]||0);
  const oppHand=12-(placed[opp]||0);
  const myBoard=onlineCount(board,myColor);
  const oppBoard=onlineCount(board,opp);
  // السكة: أنا المحصور إذا sika===myColor وما ضغطت الزر بعد
  const isSikaMe=gs?.sika===myColor&&gs?.turn!==myColor; // أنا محصور وانتظر أضغط الزر
  const isSikaMeWaiting=gs?.sika===myColor&&gs?.turn===myColor; // أنا محصور وفي دوري لكن ما أقدر أحرك
  const isSikaOpp=gs?.sika&&gs.sika!==myColor&&gs?.turn===myColor; // أنا الخصم أفتح

  const doMove=async(r,c)=>{
    if(!isMyTurn||!gs)return;
    const ph=gs.phase;
    if(ph==="placement"){
      if(board[r][c]!==null||(r===2&&c===2))return;
      if((placed[myColor]||0)>=12||(gs.tp||0)>=2)return;
      const nb=board.map(r=>[...r]);nb[r][c]=myColor;
      const np={...placed,[myColor]:(placed[myColor]||0)+1};
      const ntp=(gs.tp||0)+1;
      let nt=myColor,ntp2=ntp,nph="placement";
      if(ntp>=2){
        nt=opp;ntp2=0;
        if((np.white||0)+(np.black||0)>=24){nph="movement";nt=opp===gs.first?myColor:opp;}
      }
      await dbPatch(`rooms/${roomId}`,{board:b2s(nb),placed:np,tp:ntp2,turn:nt,phase:nph,first:gs.first||myColor,v:(gs.v||0)+1});
    } else {
      if(sel){
        const ok=vmoves.some(([mr,mc])=>mr===r&&mc===c);
        if(ok){
          const nb=board.map(r=>[...r]);
          const caps=onlineCaptures(board,sel[0],sel[1],r,c);
          nb[r][c]=myColor;nb[sel[0]][sel[1]]=null;
          for(const[cr,cc]of caps)nb[cr][cc]=null;
          setSel(null);setVmoves([]);
          const win=onlineWin(nb,myColor);
          const more=caps.length>0&&onlineCapMoves(nb,r,c).length>0;
          const nt=win?null:more?myColor:opp;
          const blocked=!win&&!more&&onlineBlocked(nb,opp);
          await dbPatch(`rooms/${roomId}`,{
            board:b2s(nb),
            // لو الخصم محصور، خلي الدور عنده عشان يضغط طلب سكة
            turn:blocked?opp:nt,
            winner:win||null,
            locked:more?`${r},${c}`:null,
            sika:blocked?opp:null,
            v:(gs.v||0)+1
          });
        } else if(board[r][c]===myColor){
          const lk=gs.locked?gs.locked.split(",").map(Number):null;
          if(lk&&!(lk[0]===r&&lk[1]===c))return;
          setSel([r,c]);setVmoves(onlineValidMoves(board,r,c));
  ...
  const doSikaOpp=async(r,c)=>{
    if(!isSikaOpp)return;
    if(!sikaFrom){
      if(board[r][c]!==myColor)return;
      if(onlineValidMoves(board,r,c).length===0)return;
      setSikaFrom([r,c]);
    } else {
      if(board[r][c]!==null)return;
      const dr=Math.abs(r-sikaFrom[0]),dc=Math.abs(c-sikaFrom[1]);
      if(!((dr===1&&dc===0)||(dr===0&&dc===1))){setSikaFrom(null);return;}
      const nb=board.map(r=>[...r]);
      nb[r][c]=myColor;nb[sikaFrom[0]][sikaFrom[1]]=null;
      setSikaFrom(null);
      const blockedPlayer = gs.sikaRequester || gs.sika;
      await dbPatch(`rooms/${roomId}`,{
        board:b2s(nb),
        turn:blockedPlayer,
        sika:null,
  ...
  const click=(r,c)=>{
    if(isSikaOpp){doSikaOpp(r,c);return;}
    doMove(r,c);
  };

  let msg="جاري التحميل...";
  if(oppGone)msg="⚠️ الخصم خرج!";
  else if(gs){
    if(gs.winner)msg=gs.winner===myColor?"🏆 فزت! 🎉":"💔 خسرت!";
    else if(isSikaMeWaiting)msg="أنت محصور! اضغط 🚪 طلب سكة";
    else if(isSikaMe)msg="انتظر... الخصم يفتح لك الطريق";
    else if(isSikaOpp)msg=sikaFrom?"اختر مربعاً فاضياً مجاوراً":"اختر أحد حجارك لتفتح السكة";
    else if(isMyTurn)msg=gs.phase==="placement"?`دورك ← ضع حجرين (${myHand} يد)`:"دورك ← اختر حجراً";
    else msg="⏳ دور خصمك...";
  }

  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:10,width:"100%",maxWidth:420}}>
      {oppGone&&<div style={{background:"rgba(200,80,20,0.2)",border:"1px solid rgba(220,100,40,0.4)",borderRadius:10,padding:"7px 16px",color:"#ffb080",fontFamily:"'Cairo',sans-serif",fontSize:"0.75rem",textAlign:"center",width:"100%"}}>⚠️ الخصم خرج من اللعبة</div>}

      {/* كود الغرفة */}
      <div style={{background:"rgba(0,0,0,0.3)",border:"1px solid rgba(212,168,67,0.2)",borderRadius:10,padding:"5px 16px",textAlign:"center"}}>
        <span style={{color:"#5a4020",fontFamily:"'Cairo',sans-serif",fontSize:"0.6rem"}}>كود: </span>
        <span style={{color:"#f5d78a",fontFamily:"monospace",fontSize:"1rem",fontWeight:700,letterSpacing:4}}>{roomId}</span>
      </div>

      {/* لوحتا اللاعبين */}
      <div style={{display:"flex",width:"100%",gap:8}}>
        {[[myColor,myName,myHand,myBoard],[opp,"الخصم",oppHand,oppBoard]].map(([color,name,hand,onBrd],i)=>{
          const active=gs?.turn===color;
          return(
            <div key={i} style={{flex:1,padding:"7px 9px",borderRadius:11,
              background:active?"linear-gradient(135deg,rgba(212,168,67,0.12),rgba(180,130,40,0.06))":"rgba(0,0,0,0.2)",
              border:`1.5px solid ${active?"rgba(212,168,67,0.5)":"rgba(212,168,67,0.08)"}`,
              display:"flex",alignItems:"center",gap:7}}>
              <div style={{width:22,height:22,borderRadius:"50%",flexShrink:0,
                background:color==="white"?"radial-gradient(circle at 32% 28%,#FFFFF0,#C8A96E)":"radial-gradient(circle at 32% 28%,#484848,#000)",
                border:color==="white"?"1.5px solid #B89850":"1.5px solid #333"}}/>
              <div style={{flex:1}}>
                <div style={{color:active?"#C8A96E":"#4a3010",fontFamily:"'Cairo',sans-serif",fontSize:"0.6rem",fontWeight:700}}>{name}{active?" ●":""}</div>
                <div style={{display:"flex",flexWrap:"wrap",gap:1.5,marginTop:2}}>
                  {Array(12).fill(null).map((_,j)=>(
                    <div key={j} style={{width:6,height:6,borderRadius:"50%",
                      background:j<hand?(color==="white"?"radial-gradient(circle,#FFFFF0,#C8A96E)":"radial-gradient(circle,#484848,#000)"):"rgba(255,255,255,0.04)",
                      border:j<hand?(color==="white"?"1px solid #B89850":"1px solid #333"):"none"}}/>
                  ))}
                </div>
                <div style={{color:"#3a2810",fontFamily:"'Cairo',sans-serif",fontSize:"0.48rem",marginTop:1}}>{hand} يد | {onBrd} لوح</div>
              </div>
            </div>
          );
        })}
      ...
      <div style={{width:"100%"}}>
        <div style={{
          background:gs?.winner?"linear-gradient(135deg,rgba(212,168,67,0.2),rgba(160,100,20,0.15))":isSikaMe||isSikaOpp?"linear-gradient(135deg,rgba(200,80,20,0.18),rgba(150,40,10,0.12))":"rgba(0,0,0,0.3)",
          border:`1px solid ${gs?.winner?"rgba(212,168,67,0.5)":isSikaMe||isSikaOpp?"rgba(220,100,40,0.45)":"rgba(212,168,67,0.12)"}`,
          borderRadius:timerOn?"10px 10px 0 0":10,padding:"8px 16px",
          color:gs?.winner?"#f5d78a":isSikaMe||isSikaOpp?"#ffb080":isMyTurn?"#C8A96E":"#5a4020",
          fontFamily:"'Cairo',sans-serif",fontSize:"0.82rem",textAlign:"center"}}>
          {msg}
        </div>
        {timerOn&&!gs?.winner&&(
          <div style={{height:6,background:"rgba(0,0,0,0.4)",borderRadius:"0 0 10px 10px",overflow:"hidden"}}>
            <div style={{height:"100%",width:`${(timer/30)*100}%`,
              background:timer>10?"linear-gradient(90deg,#d4a843,#f5d78a)":timer>5?"linear-gradient(90deg,#ff8800,#ffbb00)":"linear-gradient(90deg,#ff3300,#ff6600)",
              transition:"width 1s linear"}}/>
          </div>
        )}
        {timerOn&&!gs?.winner&&(
          <div style={{textAlign:"center",marginTop:2}}>
            <span style={{color:timer>10?"#6a4820":timer>5?"#cc6600":"#ff4400",fontFamily:"'Cairo',sans-serif",fontSize:"0.6rem",fontWeight:700}}>⏱️ {timer}ث</span>
          </div>
        )}
      </div>

      {/* اللوحة */}
      <div style={{background:"linear-gradient(145deg,#2C1F0E,#1E1509,#251A0C)",border:"3px solid rgba(212,168,67,0.2)",borderRadius:12,padding:8,boxShadow:"0 20px 60px rgba(0,0,0,0.8)",position:"relative"}}>
        {[{top:5,right:5},{top:5,left:5},{bottom:5,right:5},{bottom:5,left:5}].map((s,i)=>(
          <div key={i} style={{position:"absolute",width:13,height:13,...s,
            borderTop:[0,1].includes(i)?"1.5px solid rgba(212,168,67,0.35)":"none",
            borderBottom:[2,3].includes(i)?"1.5px solid rgba(212,168,67,0.35)":"none",
            borderRight:[0,2].includes(i)?"1.5px solid rgba(212,168,67,0.35)":"none",
            borderLeft:[1,3].includes(i)?"1.5px solid rgba(212,168,67,0.35)":"none"}}/>
        ))}
        {board.map((row,r)=>(
          <div key={r} style={{display:"flex"}}>
            {row.map((piece,c)=>{
              const cnt=r===2&&c===2;
              const sf=safe.has(`${r},${c}`);
              const vm=vmoves.some(([mr,mc])=>mr===r&&mc===c);
              const lk=gs?.locked?gs.locked.split(",").map(Number):null;
              const dim=lk&&piece===myColor&&!(lk[0]===r&&lk[1]===c)&&gs?.turn===myColor;
              let bg=(r+c)%2===0?"rgba(255,255,255,0.025)":"rgba(0,0,0,0.12)";
              let border="rgba(212,168,67,0.07)";
              if(cnt){bg="rgba(212,168,67,0.08)";border="rgba(212,168,67,0.3)";}
              else if(sf){bg="rgba(212,168,67,0.04)";border="rgba(212,168,67,0.2)";}
              if(vm&&!piece){bg="rgba(212,168,67,0.15)";border="rgba(212,168,67,0.6)";}
              if(isSikaOpp&&!sikaFrom&&board[r][c]===myColor&&onlineValidMoves(board,r,c).length>0){border="rgba(255,150,50,0.7)";bg="rgba(200,80,20,0.1)";}
              if(isSikaOpp&&sikaFrom&&piece===null&&!cnt){border="rgba(100,220,100,0.5)";bg="rgba(50,150,50,0.08)";}
              if(sikaFrom&&sikaFrom[0]===r&&sikaFrom[1]===c){border="rgba(255,200,50,0.8)";bg="rgba(200,160,20,0.15)";}
              return(
                <div key={c} onClick={()=>click(r,c)} style={{width:C,height:C,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",cursor:(isMyTurn||isSikaOpp)?"pointer":"default",touchAction:"manipulation",WebkitTapHighlightColor:"transparent",background:bg,border:`1px solid ${border}`,borderRadius:3,boxSizing:"border-box",opacity:dim?0.3:1,transition:"opacity 0.2s"}}>
                  {cnt&&!piece&&<div style={{width:12,height:12,background:"rgba(212,168,67,0.5)",clipPath:"polygon(50% 0%,61% 35%,98% 35%,68% 57%,79% 91%,50% 70%,21% 91%,32% 57%,2% 35%,39% 35%)"}}/>}
                  {sf&&!cnt&&!piece&&<div style={{position:"absolute",inset:3,borderRadius:3,border:"1px solid rgba(212,168,67,0.18)",pointerEvents:"none"}}/>}
                  ...
                  {piece&&<div style={{zIndex:3,width:P,height:P,borderRadius:"50%",position:"relative",overflow:"hidden",flexShrink:0,
                    background:piece==="white"?"radial-gradient(circle at 32% 28%,#FFFFF0,#E8D9A8,#C8A96E)":"radial-gradient(circle at 32% 28%,#484848,#1a1a1a,#000)",
                    border:sel&&sel[0]===r&&sel[1]===c?"3px solid #f5d78a":piece==="white"?"1.5px solid #B89850":"1.5px solid #404040",
                    boxShadow:sel&&sel[0]===r&&sel[1]===c?"0 0 0 3px rgba(212,168,67,0.3)":piece==="white"?"0 4px 12px rgba(0,0,0,0.5)":"0 4px 12px rgba(0,0,0,0.7)"}}>
                    <div style={{position:"absolute",top:"10%",left:"15%",width:"35%",height:"25%",borderRadius:"50%",background:piece==="white"?"rgba(255,255,255,0.7)":"rgba(255,255,255,0.15)",filter:"blur(2px)"}}/>
                  </div>}
                </div>
              );
            })}
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",justifyContent:"center"}}>
        {(isSikaMeWaiting)&&!gs?.winner&&(
          <button onClick={async()=>{
            await dbPatch(`rooms/${roomId}`,{
              turn:opp,
              sikaRequester:myColor,
              v:(gs.v||0)+1
            });
          }} style={{padding:"5px 14px",borderRadius:20,background:"linear-gradient(135deg,rgba(200,60,20,0.5),rgba(150,40,10,0.3))",border:"1px solid rgba(220,100,40,0.5)",color:"#ffb080",fontFamily:"'Cairo',sans-serif",fontSize:"0.72rem",cursor:"pointer",fontWeight:700,animation:"sikaBeat 1s infinite"}}>🚪 طلب سكة</button>
        )}
        <style>{`@keyframes sikaBeat{0%,100%{box-shadow:0 0 8px rgba(220,80,20,.4)}50%{box-shadow:0 0 20px rgba(220,80,20,.8)}}`}</style>
        {gs?.winner&&(
          <button onClick={()=>{
            dbPatch(`rooms/${roomId}`,{board:emptyStr(),phase:"placement",turn:"white",placed:{white:0,black:0},tp:0,first:null,winner:null,locked:null,sika:null,v:(gs.v||0)+1});
          }} style={{padding:"5px 14px",borderRadius:20,background:"rgba(212,168,67,0.1)",border:"1px solid rgba(212,168,67,0.3)",color:"#C8A96E",fontFamily:"'Cairo',sans-serif",fontSize:"0.72rem",cursor:"pointer"}}>🔄 جديد</button>
        )}
        <button onClick={onLeave} style={{padding:"5px 14px",borderRadius:20,background:"rgba(200,50,50,0.12)",border:"1px solid rgba(200,50,50,0.3)",color:"#e08080",fontFamily:"'Cairo',sans-serif",fontSize:"0.72rem",cursor:"pointer"}}>← رجوع</button>
      </div>
      <style>{`@keyframes vmP{0%,100%{transform:scale(1)}50%{transform:scale(1.25)}}`}</style>
    </div>
  );
}
...
function OnlineLobby({playerName,onBack}){
  ...
  const create=async()=>{
    ...
    const ok=await dbWrite(`rooms/${code}`,{
      board:emptyStr(),phase:"placement",turn:"white",
      placed:{white:0,black:0},tp:0,first:null,winner:null,
      locked:null,sika:null,
      players:{white:playerName,black:""},hb:{},v:0
    });
    if(!ok){setError("فشل إنشاء الغرفة - تحقق من الاتصال");setLoading(false);return;}
    setRoomId(code);setMyColor("white");setScreen("waiting");
    pollRef.current=setInterval(async()=>{
      const val=await dbRead(`rooms/${code}/players/black`);
      if(val&&val!==""){clearInterval(pollRef.current);setScreen("game");}
    },1500);
    setLoading(false);
  };
  ...
  const join=async()=>{
    ...
    setLoading(true);setError("");
    const data=await dbRead(`rooms/${code}`);
    if(!data){setError("الغرفة غير موجودة!");setLoading(false);return;}
    if(data.players?.black&&data.players.black!==""){setError("الغرفة ممتلئة!");setLoading(false);return;}
    await dbWrite(`rooms/${code}/players/black`,playerName);
  ...
  const btn=(p=true)=>({width:"100%",padding:"11px",borderRadius:12,cursor:loading?"not-allowed":"pointer",fontWeight:700,fontFamily:"'Cairo',sans-serif",fontSize:"0.82rem",background:p?"linear-gradient(135deg,rgba(212,168,67,0.2),rgba(160,100,20,0.14))":"rgba(0,0,0,0.3)",border:`1.5px solid ${p?"rgba(212,168,67,0.5)":"rgba(212,168,67,0.1)"}`,color:p?"#f5d78a":"#5a4020",opacity:loading?0.6:1});

  if(screen==="game")return<OnlineBoard roomId={roomId} myColor={myColor} myName={playerName} onLeave={()=>{setScreen("lobby");setRoomId("");}}/>;
  ...
  if(screen==="waiting")return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:14,maxWidth:300,width:"100%"}}>
      <div style={{...card,textAlign:"center"}}>
        ...
        <p style={{color:"#5a4020",fontFamily:"'Cairo',sans-serif",fontSize:"0.68rem",margin:"0 0 14px"}}>شارك الكود مع صديقك</p>
        <div style={{background:"rgba(212,168,67,0.08)",border:"2px dashed rgba(212,168,67,0.35)",borderRadius:14,padding:"14px",marginBottom:12}}>
        ...
        <div style={{display:"flex",gap:8,justifyContent:"center"}}>{[0,1,2].map(i=><div key={i} style={{width:8,height:8,borderRadius:"50%",background:"rgba(212,168,67,0.5)",animation:`dot ${0.8+i*0.2}s ${i*0.15}s infinite`}}/>)}</div>
      </div>
      <button onClick={()=>{clearInterval(pollRef.current);dbWrite(`rooms/${roomId}`,null);setScreen("lobby");}} style={{...btn(false),width:"auto",padding:"6px 18px"}}>إلغاء</button>
      <style>{`@keyframes dot{0%,100%{transform:scale(1)}50%{transform:scale(1.4)}}`}</style>
    </div>
  );

  return(
    <div style={{display:"flex",flexDirection:"column",gap:14,maxWidth:300,width:"100%"}}>
      <div style={{textAlign:"center"}}>
        <div style={{fontSize:"2rem"}}>🌐</div>
        <h3 style={{color:"#C8A96E",fontFamily:"'Amiri',serif",fontSize:"1rem",margin:"6px 0 3px"}}>اللعب أونلاين</h3>
        <p style={{color:"#3a2810",fontFamily:"'Cairo',sans-serif",fontSize:"0.62rem",margin:0}}>العب مع أصدقائك في أي مكان</p>
      </div>
      <div style={card}>
        <h4 style={{color:"#C8A96E",fontFamily:"'Cairo',sans-serif",fontSize:"0.78rem",margin:"0 0 8px",textAlign:"center"}}>🏠 إنشاء غرفة جديدة</h4>
        <p style={{color:"#5a4020",fontFamily:"'Cairo',sans-serif",fontSize:"0.62rem",margin:"0 0 12px",textAlign:"center"}}>أنشئ غرفة وشارك الكود مع صديقك</p>
        <button onClick={create} disabled={loading} style={btn()}>{loading?"جاري الإنشاء...":"+ إنشاء غرفة"}</button>
      </div>
      <div style={card}>
        <h4 style={{color:"#C8A96E",fontFamily:"'Cairo',sans-serif",fontSize:"0.78rem",margin:"0 0 8px",textAlign:"center"}}>🔗 انضم لغرفة</h4>
        <div style={{color:"#6a4820",fontFamily:"'Cairo',sans-serif",fontSize:"0.65rem",marginBottom:6}}>أدخل كود الغرفة</div>
        <input value={joinCode} onChange={e=>{setJoinCode(e.target.value.toUpperCase());setError("");}} placeholder="XXXXXX" maxLength={6}
          style={{width:"100%",padding:"10px",borderRadius:10,background:"rgba(0,0,0,0.4)",border:"1px solid rgba(212,168,67,0.3)",color:"#C8A96E",fontFamily:"monospace",fontSize:"1.1rem",textAlign:"center",outline:"none",letterSpacing:4,boxSizing:"border-box"}}/>
        {error&&<div style={{color:"#ff6060",fontFamily:"'Cairo',sans-serif",fontSize:"0.65rem",marginTop:6,textAlign:"center"}}>{error}</div>}
        <button onClick={join} disabled={loading} style={{...btn(),marginTop:10}}>{loading?"جاري الانضمام...":"انضم ←"}</button>
      </div>
  ...
  return(
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:12}}>
      ...
      {profile&&(()=>{
        const lvl=getPlayerLevel(profile.totalPoints||0);
        return(
          <div style={{width:"100%",maxWidth:260,background:"linear-gradient(135deg,rgba(212,168,67,0.1),rgba(100,60,10,0.07))",border:"1.5px solid rgba(212,168,67,0.2)",borderRadius:14,padding:"10px 14px",marginBottom:4,display:"flex",alignItems:"center",gap:10,backdropFilter:"blur(6px)",WebkitBackdropFilter:"blur(6px)",cursor:"pointer"}} onClick={()=>onSelect("profile")}>
      ...
      <div style={{display:"flex",gap:14}}>
        {[WHITE,BLACK].map(color=>{
          ...
          <button key={color} onClick={()=>onSelect(color,selTheme)} style={{width:110,padding:"14px 10px",borderRadius:14,
            ...
            cursor:"pointer",display:"flex",flexDirection:"column",alignItems:"center",gap:8,backdropFilter:"blur(8px)",WebkitBackdropFilter:"blur(8px)",transition:"all .2s"}}
...
function loadData() {
  try {
    const raw = (typeof localStorage!=="undefined"&&localStorage.getItem("seja_data"))||"{}";
...
function saveData(d) {
  try {
    if(typeof localStorage!=="undefined"){
      localStorage.setItem("seja_data", JSON.stringify(d));
    }
  } catch(e){}
}
    ...
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0F0A05 0%,#1A0F07 40%,#120B04 70%,#0A0602 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Amiri','Cairo',serif",direction:"rtl",padding:16,userSelect:"none"}}>
      <style>{`
        ...
        *{box-sizing:border-box;}
        ...
        @-webkit-keyframes pulse{0%,100%{-webkit-transform:scale(1)}50%{-webkit-transform:scale(1.25)}}
        @keyframes pulse{0%,100%{transform:scale(1)}50%{transform:scale(1.25)}}
        
        @-webkit-keyframes piece-drop{0%{-webkit-transform:scale(0) rotate(-180deg);opacity:0}100%{-webkit-transform:scale(1) rotate(0);opacity:1}}
        @keyframes piece-drop{0%{transform:scale(0)rotate(-180deg);opacity:0}100%{transform:scale(1)rotate(0);opacity:1}}
        
        @-webkit-keyframes piece-shake{0%,100%{-webkit-transform:translateX(0)}20%{-webkit-transform:translateX(-4px)}40%{-webkit-transform:translateX(4px)}60%{-webkit-transform:translateX(-2px)}80%{-webkit-transform:translateX(2px)}}
        ...
        @-webkit-keyframes shimmer{0%{background-position:200% center}100%{background-position:-200% center}}
        @keyframes shimmer{0%{background-position:200% center}100%{background-position:-200% center}}
        
        @-webkit-keyframes beat{0%,100%{box-shadow:0 0 6px rgba(212,168,67,.5)}50%{box-shadow:0 0 16px rgba(212,168,67,.9)}}
        @keyframes beat{0%,100%{box-shadow:0 0 6px rgba(212,168,67,.5)}50%{box-shadow:0 0 16px rgba(212,168,67,.9)}}
        
        @-webkit-keyframes sikaBeat{0%,100%{box-shadow:0 0 8px rgba(220,80,20,.4)}50%{box-shadow:0 0 20px rgba(220,80,20,.8)}}
        @keyframes sikaBeat{0%,100%{box-shadow:0 0 8px rgba(220,80,20,.4)}50%{box-shadow:0 0 20px rgba(220,80,20,.8)}}
        
        @-webkit-keyframes centerGlow{0%,100%{box-shadow:0 0 8px rgba(212,168,67,.22)}50%{box-shadow:0 0 20px rgba(212,168,67,.5)}}
        @keyframes centerGlow{0%,100%{box-shadow:0 0 8px rgba(212,168,67,.22)}50%{box-shadow:0 0 20px rgba(212,168,67,.5)}}
        
        @-webkit-keyframes starSpin{0%{-webkit-transform:rotate(0)}100%{-webkit-transform:rotate(360deg)}}
        @keyframes starSpin{0%{transform:rotate(0)}100%{transform:rotate(360deg)}}
        
        @-webkit-keyframes confettiFall{0%{-webkit-transform:translateY(-20px) rotate(0);opacity:1}100%{-webkit-transform:translateY(100vh) rotate(720deg);opacity:0}}
        ...
        @-webkit-keyframes comboAnim{0%{-webkit-transform:translateX(-50%) scale(0)}100%{-webkit-transform:translateX(-50%) scale(1)}}
        @keyframes comboAnim{0%{transform:translateX(-50%)scale(0)}100%{transform:translateX(-50%)scale(1)}}
        
        @-webkit-keyframes badgeIn{0%{-webkit-transform:translateX(-50%) translateY(-20px) scale(0.5);opacity:0}100%{-webkit-transform:translateX(-50%) translateY(0) scale(1);opacity:1}}
        ...
        @-webkit-keyframes float{0%,100%{-webkit-transform:translateY(0)}50%{-webkit-transform:translateY(-6px)}}
        @keyframes float{0%,100%{transform:translateY(0)}50%{transform:translateY(-6px)}}
        
        @keyframes capFlash{0%{opacity:1}100%{opacity:0}}
        
        .vm-cell{-webkit-animation:pulse .85s infinite;animation:pulse .85s infinite;cursor:pointer;}
        .sika-cell{-webkit-animation:pulse .7s infinite;animation:pulse .7s infinite;cursor:pointer;}
        .piece-drop{-webkit-animation:piece-drop .4s cubic-bezier(.34,1.56,.64,1) forwards;animation:piece-drop .4s cubic-bezier(.34,1.56,.64,1) forwards;}
        .piece-shake{-webkit-animation:piece-shake .4s ease;animation:piece-shake .4s ease;}
        
        /* iOS Safari fixes */
        input{-webkit-appearance:none;appearance:none;}
        button{-webkit-appearance:none;appearance:none;touch-action:manipulation;}
        
        /* Prevent text selection on game board */
        .game-board{-webkit-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none;}
      ...
      <div style={{marginBottom:screen==="menu"?20:12,textAlign:"center"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:12}}>
          ...
          <h1 style={{fontSize:"clamp(1.8rem,5vw,2.8rem)",margin:0,fontFamily:"'Amiri',serif",fontWeight:700,letterSpacing:2,background:"linear-gradient(135deg,#f5d78a,#d4a843,#a07030,#d4a843,#f5d78a)",backgroundSize:"300% auto",WebkitBackgroundClip:"text",backgroundClip:"text",WebkitTextFillColor:"transparent",color:"transparent",animation:"shimmer 5s linear infinite"}}>سيجة</h1>
          <span style={{color:"rgba(212,168,67,0.35)",fontFamily:"monospace",fontSize:"clamp(0.7rem,2vw,0.9rem)",letterSpacing:2}}>seja</span>
        </div>
