...
function OnlineBoard({roomId,myColor,playerName,onLeave}){
  ...
  const handleCell=async(r,c)=>{
    ...
    if(gs.phase==="placement"){
      ...
      if(sel){
        const valid=vmoves.some(([mr,mc])=>mr===r&&mc===c);
        if(valid){
          const nb=board.map(r=>[...r]);const caps=getCaptures(board,sel[0],sel[1],r,c);
  ...
  const myInHand=12-(gs?.placed?.[myColor]||0),oppInHand=12-(gs?.placed?.[oppColor]||0);
...
function OnlineLobby({playerName,onBack}){
  ...
  const joinRoom=async()=>{
    ...
    try{
      ...
      if(data.players?.black){setError("الغرفة ممتلئة!");setLoading(false);return;}
