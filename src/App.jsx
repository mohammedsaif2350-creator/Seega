...
function Game({mode,onBack,playerColor,aiLevel,scores,onAddPoints,pieceTheme}){
...
export default function AlSija(){
  ...
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(160deg,#0F0A05 0%,#1A0F07 40%,#120B04 70%,#0A0602 100%)",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",fontFamily:"'Amiri','Cairo',serif",direction:"rtl",padding:16,userSelect:"none"}}>
      ...
      {screen==="menu" && <Menu onSelect={m=>{if(m==="tutorial"){setScreen("tutorial");}else{setMode(m);if(m==="vs-ai")setScreen("level");else{setScreen("color-2p");}}}} scores={scores}/>}
      ...
      {screen==="game" && <ErrorBoundary><Game mode={mode} onBack={()=>setScreen("menu")} playerColor={playerColor} aiLevel={aiLevel} scores={scores} onAddPoints={addPoints} pieceTheme={pieceTheme}/></ErrorBoundary>}
    </div>
  );
}
